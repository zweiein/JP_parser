...
def ReadAlignFormat(file_path_name) :
    ...
    for element in split_list :
        if element[-1] == '+' :
            processed_list.append(element[0:len(element)-1])
        if element[-1] != '+' and len(element) >= 4 :
...
if __name__ == '__main__':
    ...
    for this_dir in doc_dir_names:
        ...
        total_lines_widtxt = sum(1 for line in open(wordid_docs_path + this_dir + '.sa.txt', 'r'))
        ...
        print len(slide_and_sentnum_tuple)
        curret_slide = 0
        for next_slide in range(1, len(slide_and_sentnum_tuple)+1):

            slide_wid_fp = open(output_path + this_dir + '.' + '#' + str(next_slide) + '.txt', 'w')

            #下一個跟目前的一樣，表示現在這個slide應該要結束
            if next_slide != len(slide_and_sentnum_tuple) and slide_and_sentnum_tuple[next_slide] == curret_slide :
                curret_slide = slide_and_sentnum_tuple[next_slide]
                slide_wid_fp.close()
            else : #否則就是正常的狀況
                #如果下一個slidenum已經到底了(舉例來說：總共只有36個slide，目前是34；但下一個是35，已經撞到list的底部)
                if curret_slide == len(slide_and_sentnum_tuple) - 1 :
                    for line_num in range(int(slide_and_sentnum_tuple[curret_slide]), total_lines_widtxt) :
                        slide_wid_fp.write(wid_fp.readline())
                    #end for lines in wid txt
                    slide_wid_fp.close()
                    curret_slide = curret_slide + 1
                #end if
                else : #否則就是正常狀況
                    distance = int(slide_and_sentnum_tuple[next_slide]) - int(slide_and_sentnum_tuple[curret_slide])
                    for line_num in range(int(slide_and_sentnum_tuple[curret_slide]), int(slide_and_sentnum_tuple[curret_slide])+distance) :
                        slide_wid_fp.write(wid_fp.readline())
                    #end for lines in wid txt
                    slide_wid_fp.close()
                    curret_slide = curret_slide + 1
